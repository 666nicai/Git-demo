...
void searchPatientByName() { // 按姓名模糊查询患者信息
    clearScreen();
    drawModernBox(25, 5, 45, 8, " 查 询 患 者 ");
    char name[MAX_NAME_LEN];
    gotoxy(30, 8); printf("请输入患者姓名: "); safeInput(name, 20, 46, 8);
...
void searchDoctorByName() {
    ...
    char name[MAX_NAME_LEN];
    gotoxy(30, 8); printf("请输入医生姓名: "); safeInput(name, 20, 46, 8);
    Doctor* d = doctorHead;
    int found = 0;
    clearScreen();
    setColor(COLOR_TITLE);
...
void searchMedicineByName() {
    ...
    gotoxy(30, 8); printf("请输入药品名: "); safeInput(name, 20, 44, 8);
