...
void listPrescriptions() {
    ...
    // 若为医生角色，只收集自己的处方
    Prescription* displayHead = prescriptionHead;
    Prescription* filteredList = NULL;
    Prescription* filteredTail = NULL;
    if (currentUserRole == ROLE_DOCTOR) {
        Prescription* p = prescriptionHead;
        while (p) {
            if (p->doctorId == currentDoctorId) {
                // 复制节点（仅复制处方头，不复制 items，items 保持原样）
                Prescription* copy = (Prescription*)malloc(sizeof(Prescription));
                memcpy(copy, p, sizeof(Prescription));
                copy->next = NULL;
    ...
    // 释放复制的临时节点
    if (currentUserRole == ROLE_DOCTOR && filteredList) {
        Prescription* f = filteredList;
        while (f) {
            Prescription* next = f->next;
            free(f);
            f = next;
        }
    }
